...
InsertCRANChecking <- function(con, checking) {
  ...
  has.status <- sapply(apply(checking, 1, GetCRANStatusKey), has.key, status)
  if (any(!has.status)) {
    warning(sprintf("%d unfound status", length(has.status[!has.status])))
    checking <- checking[has.status, ]
  }
